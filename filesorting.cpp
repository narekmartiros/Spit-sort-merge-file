...
const int number_count=500000;
const int elements_in_raw=10;
const int split_count=10;
const char* filename="file-";
constexpr int row_count=number_count/elements_in_raw;
...
void split_file(){
    ...
    while(!split.eof()){
        int small_file_size=row_count/split_count;;
        std::ofstream temp;
        std::string temp_str;
        std::getline(split,temp_str);
        if(i%small_file_size==0&&i>0){
            std::string file_name=filename+std::to_string(j);
    ...
    std::ofstream temp2;
    file_name=filename+std::to_string(j);
